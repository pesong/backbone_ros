...
namespace ssd_ros {
    ...
    void SSD_Detector::init_ncs() {
        ...
        //申请 GRAPH_FILE_NAME 的内存空间，并将graphpath的地址 复制 给 GRAPH_FILE_NAME
...
//        demoClasses_ = classes;
        ...
        objectPublisher_ = nodeHandle_.advertise<std_msgs::Int8>(objectDetectorTopicName,
                                                                 ...
                                                                 objectDetectorLatch);
        boundingBoxesPublisher_ = nodeHandle_.advertise<backbone::BoundingBoxes>(
        ...
        if(total_count > 0) {
            ...
            for (int i = 1; i < numClasses_; i++) { //对每一类
                if (rosBoxCounter_[i] > 0) { //如果有框
                    backbone::BoundingBox boundingBox;
                    //把每个框放进来
